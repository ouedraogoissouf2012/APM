"""Unit tests for the TTS cache (#123).
...
(decorator/DIP) and is bounded so it can't grow without limit.
"""
...
async def test_first_call_synthesizes_and_returns_bytes():
    inner = _CountingTts()
    tts = CachingTtsProvider(inner, max_entries=8)
...
async def test_cache_is_bounded_and_evicts_least_recently_used():
    inner = _CountingTts()
    tts = CachingTtsProvider(inner, max_entries=2)
